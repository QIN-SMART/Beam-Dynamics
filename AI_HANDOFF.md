...
- 双状态源：ued_beamline_v2 中 `step>=4`（CLI 语义）与 `config.space_charge.enabled`（统一输出）并存，可能出现展示与输出不一致
- AG 侧无 SC 状态追踪：Ne 注入无校验、无"SC ON 但模型未生效"的告警
- 设计稿：`SC_integration_audit.md` §5；未实施
